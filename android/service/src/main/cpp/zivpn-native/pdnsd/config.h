 ...
 *
 * FIX: Changed from TCP_ONLY to UDP_TCP for better compatibility with standard DNS servers.
 * TCP_ONLY causes slowness with DNS servers that primarily respond via UDP (port 53).
 * UDP_TCP tries UDP first (faster), then falls back to TCP if response is truncated. */
...
/* FIX: Increased from 5 to 10 seconds for better handling of high-latency mobile networks */
...
/* FIX: Increased from 4 to 6 for faster parallel resolution with multiple upstream DNS */
#define PAR_QUERIES   6
