...
globparm_t global={
  ...
  proc_limit:        40,
  procq_limit:       60,
  /* FIX: Increased tcp_qtimeout for mobile networks with high latency */
  tcp_qtimeout:      15,
  /* FIX: Set global timeout to 8 seconds (was 0 = no limit).
   * Prevents hanging when DNS server is unresponsive. */
  timeout:           8,
  par_queries:       PAR_QUERIES,
  /* FIX: Explicit UDP_TCP for better DNS compatibility */
...
servparm_t serv_presets={
  port:              53,
  uptest:            C_NONE,
  /* FIX: Reduced server timeout from 120s to 10s.
   * 120s is too long - causes perceived "slowness" when server doesn't respond. */
  timeout:           10,
  /* FIX: Reduced from 900s (15min) to 300s (5min) for faster server testing */
  interval:          300,
  ping_timeout:      600,
  scheme:            "",
  uptest_cmd:        NULL,
  ...
  nocache:           0,
  lean_query:        1,
  edns_query:        1, /* CHANGED: Enabled EDNS0 by default to prevent Truncation and Format Error on modern DNS like Cloudflare */
  is_proxy:          0,
  rootserver:        0,
  rand_servers:      0,
  preset:            1,
