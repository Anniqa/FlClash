...
int main(int argc,char *argv[])
{
	...
	if (setrlimit(RLIMIT_NOFILE, &rlim) != 0) {
		fprintf(stderr, "Warning: Could not set RLIMIT_NOFILE: %s\n", strerror(errno));
	}
...
#undef thrdfail
	...
	/* Attempt to save cache on fatal signals too, might save some data */
	write_disk_cache();
	destroy_cache();
	
	if(sig) log_warn("Caught signal %i. Exiting.",sig);
	if (sig==SIGSEGV || sig==SIGILL || sig==SIGBUS) {
		/* Try to write cache one last time if not already done */
		crash_msg("This is a fatal signal probably triggered by a bug. Cache sync attempted.");
	}
	if (ping_isocket!=-1)
		close(ping_isocket);
#ifdef ENABLE_IPV6
