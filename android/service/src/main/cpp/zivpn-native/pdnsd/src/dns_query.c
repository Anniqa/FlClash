...
#include <fcntl.h>
#include <ctype.h>
#include <netinet/tcp.h>  /* FIX: Added for TCP_NODELAY */
#include "list.h"
#include "consts.h"
#include "ipvers.h"
#include "dns_query.h"
...
/* FIX: Optimized socket buffers for mobile/VPN networks with high latency.
 * Increased buffer size and added TCP_NODELAY to reduce latency. */
static void tune_socket_buf(int sock) {
	int bufsize = 262144; /* 256KB - better for burst queries on mobile */
	setsockopt(sock, SOL_SOCKET, SO_RCVBUF, &bufsize, sizeof(bufsize));
	setsockopt(sock, SOL_SOCKET, SO_SNDBUF, &bufsize, sizeof(bufsize));
	
	/* Disable Nagle's algorithm for faster TCP responses */
	int nodelay = 1;
 ...
 tryagain:
	...
	retry_tcp_connect:
		...
		if (connect(st->sock,SOCK_ADDR(st),SIN_LEN)==-1) {
			if (errno==EINPROGRESS || errno==EPIPE) {
				...
				if(!run_ipv4 && (errno==ENETUNREACH || errno==ENETDOWN || errno==EHOSTUNREACH || errno==ETIMEDOUT)
	...
	retry_udp_connect:
		...
		if (connect(st->sock,SOCK_ADDR(st),SIN_LEN)==-1) {
			...
			/* if IPv6 connectivity is for some reason unavailable, perhaps the
			   IPv4 fallback address can still be reached. */
			else if(!run_ipv4 && (errno==ENETUNREACH || errno==ENETDOWN || errno==EHOSTUNREACH || errno==ETIMEDOUT)
				   && st->a4fallback.s_addr!=INADDR_ANY)
			{
#if DEBUG>0
				char abuf[ADDRSTR_MAXLEN];
...
#endif
				int udp_errno = errno;
				DEBUG_PDNSDA_MSG("Error while connecting to %s: %s\n", PDNSDA2STR(PDNSD_A(st)),strerror(udp_errno));
				close(st->sock);
#if !defined(NO_TCP_QUERIES) && !defined(NO_UDP_QUERIES)
				if(st->qm==UDP_TCP &&
				   (udp_errno==ECONNRESET || udp_errno==ECONNREFUSED ||
				    udp_errno==EHOSTUNREACH || udp_errno==ENETUNREACH)) {
					switch_to_tcp(st);
					DEBUG_PDNSDA_MSG("UDP connect to %s failed. Trying TCP fallback.\n", PDNSDA2STR(PDNSD_A(st)));
					goto tryagain;
				}
#endif
				break;
			...
			if (send(st->sock,&st->msg->hdr,st->transl,0)==-1) {
				...
				if(st->qm==UDP_TCP &&
				   (udp_errno==ECONNRESET || udp_errno==ECONNREFUSED ||
				    udp_errno==EHOSTUNREACH || udp_errno==ENETUNREACH)) {
					switch_to_tcp(st);
					DEBUG_PDNSDA_MSG("UDP send to %s failed. Trying TCP fallback.\n", PDNSDA2STR(PDNSD_A(st)));
					goto tryagain;
				}
#endif
				break;
	...
	case QS_UDPRECEIVE:
	{
		...
		if ((rv=recv(st->sock,st->recvbuf,udpbufsize,0))==-1) {
			st->s_errno=errno;
				/* FIX: Handle transient errors on connected UDP sockets.
				 * ECONNRESET/ECONNREFUSED can occur when ISP sends ICMP errors
				 * or intercepts DNS traffic. Fall back to TCP immediately in UDP_TCP mode.
				 * This is especially important for VPN scenarios where UDP DNS
				 * may be blocked but TCP DNS works through the tunnel. */
				if (errno == ECONNRESET || errno == ECONNREFUSED || 
				    errno == EHOSTUNREACH || errno == ENETUNREACH) {
					DEBUG_PDNSDA_MSG("UDP error from %s: %s (will try TCP fallback)\n", 
							 PDNSDA2STR(PDNSD_A(st)), strerror(errno));
					close(st->sock);
#if !defined(NO_TCP_QUERIES) && !defined(NO_UDP_QUERIES)
					if(st->qm==UDP_TCP) {
...
int query_uptest(pdnsd_a *addr, int port, const unsigned char *name, time_t timeout, int rep)
{
	...
	qs.auth_serv=0;
	qs.lean_query=1;
	qs.edns_query=1; /* ZIVPN FIX: Enabled EDNS0 by default to prevent Truncation loops and Format Errors on Cloudflare */
	qs.needs_testing=0;
	qs.trusted=1;
	qs.aa=0;
	qs.tc=0;
